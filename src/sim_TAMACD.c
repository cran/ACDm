...
SEXP sim_TAMACD(SEXP N,
  ...
  double omega[M + 1], alpha[p * (M + 1)], nu[p * (M + 1)], beta[q * (M + 1)]; //parameters
  
  for(j = 0; j < (M + 1); j++) omega[j] = xpar[j];
  for(j = 0; j < (M + 1) * p; j++) alpha[j] = xpar[(M + 1) + j];
  for(j = 0; j < (M + 1) * r; j++) nu[j] = xpar[(p + 1) * (M + 1) + j];
  for(j = 0; j < (M + 1) * q; j++) beta[j] = xpar[(p + r + 1) * (M + 1) + j];
  ...
  for(i = Nstart; i < INTEGER(N)[0] + INTEGER(Nburn)[0]; i++){
    ...
    //adds the "r"-part:
    for(int j = 0; j < r; j++) mu[i] += nu[j * (M + 1) + regime[i]] * xe[i - j - 1];
